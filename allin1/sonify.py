...
def _sonify(
  ...
  if out_dir is not None:
    ...
    # ValueError: Invalid suffix for path: .sonif
    ...
    # Added mp3 extension to avoid errors
    demucs.separate.save_audio(
      wav=torch.from_numpy(mixed),
      path=out_dir / f'{result.path.stem}.sonif.mp3',  # Changed extension and naming convention
      samplerate=sr,
    )
